...
int main() {
    ...
    while ( rnd-- ) {
        ...
        for ( int i = 1; i <= player; i++ ) {
            ...
            do {
                ...
                if ( tmps[ tmps.length() - 1 ] == 'Q' || tmps[ tmps.length() - 2 ] == 'Q' || tmps[ tmps.length() - 3 ] == 'Q' ) {
                    ...
                    players[i].score[t] = -1;
                    players[i].dq = true;
                } else players[i].score[t] = atoi( tmps.substr( tmps.find_first_not_of(' '), tmps.length() - tmps.find_first_not_of(' ') ).c_str() ); // Store the score
            } while ( !players[i].dq );
        }
        ...
        for ( int i = 1; i <= half_num; i++ ) {
            ...
            if ( !players[i].dq )
                cout << setw(10) << left << ( tos_string( players[i].rank ) + ( ( !players[i].amateur && players[i].money > -1 && !stop && same_rank[ players[i].rank ] > 1 ) ? "T" : "" ) );
            else printf("          ");
            // Score Printing
